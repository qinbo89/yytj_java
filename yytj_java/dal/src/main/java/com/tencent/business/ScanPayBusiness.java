...
public class ScanPayBusiness {
    ...
    public ScanPayBusiness(WeiXinConfigure weiXinConfigure) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
        scanPayService = new ScanPayService(weiXinConfigure);
        scanPayQueryService = new ScanPayQueryService(weiXinConfigure);
        reverseService = new ReverseService(weiXinConfigure);
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        //同步
        if (weiXinConfigure.isUseThreadToDoReport()) {
            ReporterFactory.getReporter(weiXinConfigure,reportReqData).run();
            timeAfterReport = System.currentTimeMillis();
            log.i("pay+report总耗时（异步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        } else {
            ReportService.request(weiXinConfigure,reportReqData);
            timeAfterReport = System.currentTimeMillis();
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            //--------------------------------------------------------------------
            if (!Signature.checkIsSignValidFromResponseString(weiXinConfigure,payServiceResponseString)) {
    ...
    private boolean doOnePayQuery(String outTradeNo,ResultListener resultListener) throws Exception {
        ...
        ScanPayQueryReqData scanPayQueryReqData = new ScanPayQueryReqData(weiXinConfigure,"",outTradeNo);
        ...
        if (scanPayQueryResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(weiXinConfigure,payQueryServiceResponseString)) {
                log.e("【支付失败】支付请求API返回的数据签名验证失败，有可能数据被篡改了");
    ...
    private boolean doOneReverse(String outTradeNo,ResultListener resultListener) throws Exception {
        ...
        ReverseReqData reverseReqData = new ReverseReqData(weiXinConfigure,"",outTradeNo);
        ...
        if (reverseResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(weiXinConfigure,reverseResponseString)) {
                log.e("【支付失败】支付请求API返回的数据签名验证失败，有可能数据被篡改了");
